...
def subject_consent_on_post_save(sender, instance, raw, created, **kwargs):
    ...
    """
    if not raw:
        schedule = get_community_arm_schedule(instance)
        if schedule:
            if not created:
...
def patient_call_initial_on_post_save(sender, instance, raw, created, **kwargs):
    ...
    if not raw:
        if instance.subject_visit.schedule_name == 'intervention_visit_schedule':
...
def get_community_arm_schedule(instance=None):
        ...
        try:
            ...
            if clinician_enrollment_obj.facility in enhanced_care_communities:
                _, schedule = site_visit_schedules.get_by_onschedule_model(
                    'potlako_subject.onscheduleenhancedcare')
            elif clinician_enrollment_obj.facility in intervention_communities:
                _, schedule = site_visit_schedules.get_by_onschedule_model(
                    'potlako_subject.onscheduleintervention')
    return schedule
